...
<Callout type="warning">
	This article was written for Remix 2, which has been merged into React Router 7. Some changes may
	be necessary.
</Callout>
...
```tsx
function Emoji({ emoji, defaultChecked }: { emoji: string; defaultChecked: boolean }) {
	return (
		<label className="flex items-center justify-center">
			<input
				type="radio"
...
export function usePresenceUsers(
	...
	const streamUrl = new URL(`/content/remix-presence/example/presence`, "http://localhost:3000")
	...
	const users = userStream ? (JSON.parse(userStream) as typeof initialUsers) : initialUsers
...
export default function Example() {
	...
	const users = usePresenceUsers("/content/remix-presence/example", {
		self,
		initialUsers,
	})
